...
export const DailyGoals = ({ user }) => {
    const [goals, setGoals] = useState(HISTORY_GOALS);
    ...
    const handleSubmitProgress = () => {
        ...
        const updatedGoals = goals.map(g => {
            ...
            if (g.day === selectedGoal.day + 1) {
                return { ...g, status: 'active' }; // Change to active for user flow
            }
            return g;
        });
        ...
        <div className="w-full max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
            ...
            <div className="space-y-4">
                {goals.map((goal) => (
                    <div
                        key={goal.day}
                        className={`relative group flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${goal.status === 'active'
                    ...
                    >
                        ...
                        <div className={`relative w-16 h-16 shrink-0 flex items-center justify-center rounded-2xl transition-all duration-500 ${goal.status === 'completed'
                            ...
                            <Flame className={`w-8 h-8 ${goal.status === 'completed' ? 'text-white fill-white' : 'text-slate-500'
                                }`} />
                            <span className={`absolute -bottom-2 -right-2 w-8 h-8 flex items-center justify-center rounded-full text-xs font-bold border-4 border-slate-950 ${goal.status === 'completed' ? 'bg-green-500 text-white' : 'bg-slate-700 text-slate-400'
                                }`}>
                        ...
                        <div className="flex-1 text-center sm:text-left">
                            ...
                            <div className="flex items-center justify-center sm:justify-start gap-3 mt-2 text-xs font-medium text-slate-500">
                                ...
                                <span className={`px-2 py-1 rounded bg-slate-950 border border-slate-800 ${goal.difficulty === 'Advanced' ? 'text-red-400' : goal.difficulty === 'Intermediate' ? 'text-yellow-400' : 'text-emerald-400'
